...
if [ "$1" = "start" ] ; then
    shift
	exec /usr/lib/jvm/java-1.7.0-openjdk-1.7.0.65.x86_64/jre/bin/java -Dfile.encoding=UTF-8 -Doracle.jdbc.defaultNChar=true -classpath "neo_data_rating.jar:lib/*" -Dlog4j.configuration=file:conf/log4j.properties -Dlog4j.debug net.schedule.AutoCall >log/server.log 2>&1 &

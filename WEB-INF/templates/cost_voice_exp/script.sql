delimiter $$
create DEFINER = `billing`@`localhost` function `new_cost_voice_mpbx`(
	pid varchar(100),
	pname varchar(100),
...
begin
	...
	begin
		...
		call access('new_cost_voice_mpbx|ERR',err);
		return err;
	end;
	call access('billing.cost_voice_mpbx|NEW',concat(pid,'|',pname,'|',pfrom_minute,'|',pto_minute,'|',pprice,'|',pgroup_id,'|',pfree,'|',pnote,'|',pfrom_date,'|',pto_date));
	insert into billing.cost_voice_mpbx(id,name,from_minute,to_minute,price,group_id,free,note,from_date,to_date)
...
delimiter;
delimiter $$
create DEFINER = `billing`@`localhost` procedure `search_cost_voice_mpbx`(
	pid varchar(100),
	pname varchar(100),
	pfrom_minute varchar(100),
	pto_minute varchar(100),
...
begin
	...
	begin
		...
		@p1 = RETURNED_SQLSTATE, @p2 = MESSAGE_TEXT;
		set err:=concat('Loi thuc hien, ma loi:',@p1,', ',@p2);
		call access('search_cost_voice_mpbx|ERR',err);
		select err;
	end;
	call access('billing.cost_voice_mpbx|SEARCH',pid,'|',pname,'|',pfrom_minute,'|',pto_minute,'|',pprice,'|',pgroup_id,'|',pfree,'|',pnote,'|',pfrom_date,'|',pto_date);
	set s:='select * from billing.cost_voice_mpbx where 1=1';
...
delimiter;
delimiter $$
create DEFINER = `billing`@`localhost` function edit_cost_voice_mpbx(
	pid varchar(100),
	pname varchar(100),
	pfrom_minute varchar(100),
	pto_minute varchar(100),
...
begin
	...
	begin
		...
		@p1 = RETURNED_SQLSTATE, @p2 = MESSAGE_TEXT;
		set err:=concat('Loi thuc hien, ma loi:',@p1,', ',@p2);
		call access('edit_cost_voice_mpbx|ERR',err);
		return err;
	end;
	call access('billing.cost_voice_mpbx|EDIT',concat(pid,'|',pname,'|',pfrom_minute,'|',pto_minute,'|',pprice,'|',pgroup_id,'|',pfree,'|',pnote,'|',pfrom_date,'|',pto_date));
	update billing.cost_voice_mpbx set  id=pid,name=pname,from_minute=pfrom_minute,to_minute=pto_minute,price=pprice,group_id=pgroup_id,free=pfree,note=pnote,from_date=pfrom_date,to_date=pto_date where id=pid;
	return '1';
end $$ 
delimiter;
delimiter $$
create DEFINER = `billing`@`localhost` function del_cost_voice_mpbx(
	pId	varchar(100),
	pUserId varchar(50),
	pUserIp	varchar(50))
returns varchar(1000) charset utf8
begin
	...
	begin
		...
		@p1 = RETURNED_SQLSTATE, @p2 = MESSAGE_TEXT;
		set err:=concat('Loi thuc hien, ma loi:',@p1,', ',@p2);
		call access('del_cost_voice_mpbx|ERR',err);
		return err;
	end;
	call access('billing.cost_voice_mpbx|DEL',pId);
	delete from billing.cost_voice_mpbx where id=pid;
end;